...
if(not os.path.exists('plots/HR_Analytics/SVM')):
    dirname = 'SVM'
    os.mkdir('plots/HR_Analytics/%s' %dirname)
...
plt.figure()
title = 'HR Analytics SVM(kernel = rbf): Performance x Training Set Size'
plt.title('\n'.join(wrap(title,60)))
...
plt.savefig('plots/HR_Analytics/SVM/HRAnalytics_rbf_PerformancexTrainingSetSize.png')
print 'plot complete'
### ---
...
for c in c_coefs:
    ...
    print 'plotting results'
    plt.figure()
    title = 'HR Analytics SVM(kernel = linear, with c = ' + str(c) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/HR_Analytics/SVM/HRAnalytics_linear_' + str(c) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---
...
for d in degrees:
    ...
    print 'plotting results'
    plt.figure()
    title = 'HR Analytics SVM(kernel = poly, with degree = ' + str(d) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/HR_Analytics/SVM/HRAnalytics_poly_' + str(d) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---