...
if(not os.path.exists('plots/HandwritingVerification/SVM')):
    dirname = 'SVM'
    os.mkdir('plots/HandwritingVerification/%s' %dirname)
...
plt.figure()
title = 'Handwriting Verification SVM(kernel = rbf): Performance x Training Set Size'
plt.title('\n'.join(wrap(title,60)))
...
plt.savefig('plots/HandwritingVerification/SVM/HandwritingVerification_rbf_PerformancexTrainingSetSize.png')
print 'plot complete'
### ---
...
for c in c_coefs:
    ...
    print 'plotting results'
    plt.figure()
    title = 'Handwriting Verification SVM(kernel = linear, with c = ' + str(c) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/HandwritingVerification/SVM/HandwritingVerification_linear_' + str(c) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---
...
for d in degrees:
    ...
    print 'plotting results'
    plt.figure()
    title = 'Handwriting Verification SVM(kernel = poly, with degree = ' + str(d) +'): Performance x Training Set Size'
    plt.title('\n'.join(wrap(title,60)))
    plt.plot(offsets, test_err, '-', label='test error')
    plt.plot(offsets, train_err, '-', label='train error')
    plt.legend()
    plt.xlabel('Num Estimators')
    plt.ylabel('Mean Square Error')
    plt.savefig('plots/HandwritingVerification/SVM/HandwritingVerification_poly_' + str(d) + '_PerformancexTrainingSetSize.png')
    print 'plot complete'
    ### ---