...
if(not os.path.exists('plots/HandwritingVerification/NN')):
    dirname = 'NN'
    os.mkdir('plots/HandwritingVerification/%s' %dirname)
...
for activation in activation_functions:
    ...
    plt.figure()
    title = 'Handwriting Verification Neural Nets: Performance x Training Set Size using Activation ' + activation
    plt.title('\n'.join(wrap(title,60)))
    ...
    filename = 'HandwritingVerification_NN_' + activation + '_PerformancexTrainingSetSize.png'
    plt.savefig('plots/HandwritingVerification/NN/' + filename)
    print 'plot complete'
    ### ---