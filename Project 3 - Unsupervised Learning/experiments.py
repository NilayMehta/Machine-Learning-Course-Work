...
def create_dataset(name, test, train):
    training_set = load(train, converters["sentiment"])
    testing_set = load(test, converters["sentiment"])
...
def plot(axes, values, x_label, y_label, title, name):
    ...
    plt.ylabel(y_label)
    plt.xlabel(x_label)
    plt.savefig('dataplots/' + name + "_" + name_plot + ".png", dpi=500)
    # plt.show()
    plt.clf()
...
def pca(tx, ty, rx, ry):
    ...
    em(newtx, ty, newrx, ry, add="wPCAtr", times=24)
    km(newtx, ty, newrx, ry, add="wPCAtr", times=24)
...
def ica(tx, ty, rx, ry):
    ...
    newtx = compressor.transform(tx)
    newrx = compressor.transform(rx)
    em(newtx, ty, newrx, ry, add="wICAtr", times=24)
    km(newtx, ty, newrx, ry, add="wICAtr", times=24)
...
def randproj(tx, ty, rx, ry):
    ...
    # compressor = RandomProjection(tx[1].size)
    newrx = compressor.transform(rx)
    em(newtx, ty, newrx, ry, add="wRPtr", times=24)
    km(newtx, ty, newrx, ry, add="wRPtr", times=24)
...
def kbest(tx, ty, rx, ry):
    ...
    newtx = compressor.transform(tx)
    newrx = compressor.transform(rx)
    em(newtx, ty, newrx, ry, add="wKBtr", times=24)
    km(newtx, ty, newrx, ry, add="wKBtr", times=24)
...
if __name__=="__main__":
    ...
    print("Data processed for HR, starting")
    nn(train_x, train_y, test_x, test_y); print 'nn done HR'
    em(train_x, train_y, test_x, test_y, times = 14); print 'em done HR'
    km(train_x, train_y, test_x, test_y, times = 14); print 'km done HR'
    pca(train_x, train_y, test_x, test_y); print 'pca done HR'
    ica(train_x, train_y, test_x, test_y); print 'ica done HR'
    randproj(train_x, train_y, test_x, test_y); print 'randproj done HR'
    kbest(train_x, train_y, test_x, test_y); print 'kbest done HR'
    ...
    test = name+".test"
    train_x, train_y, test_x, test_y = create_dataset(name, test, train)
    print("Data processed for HV, starting")
    nn(train_x, train_y, test_x, test_y); print 'nn done HV'
    em(train_x, train_y, test_x, test_y, times = 24); print 'em done HV'
    km(train_x, train_y, test_x, test_y, times = 24); print 'km done HV'
    pca(train_x, train_y, test_x, test_y); print 'pca done HV'
    ica(train_x, train_y, test_x, test_y); print 'ica done HV'
    randproj(train_x, train_y, test_x, test_y); print 'randproj done HV'
    kbest(train_x, train_y, test_x, test_y); print 'kbest done HV'
